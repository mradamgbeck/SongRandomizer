...
const {randomNumberInRange, shuffleArray} = require("./randomFunctions");
...
function getScaleForKey(key, scale) {
    ...
    for (let step of interval) {
        ...
        if (notes.length < notes.indexOf(previousNote) + step + 1) {
            let wrappedIndex =
                Math.abs(step - (notes.length - notes.indexOf(previousNote)));
            scaleNotes.push(notes[wrappedIndex])
        } else {
            scaleNotes.push(notes[notes.indexOf(previousNote) + step])
        }
    }
    return {
        name: scale.name,
